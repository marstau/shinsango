...
CFont::CFont(const Filesystem::AbsolutePath & file):
type(Fixed),
width(0),
height(0),
spacingx(0),
...
pcxsize(0){
    if (Mugen::Util::fixCase(file.getExtension()) != "fnt"){
        throw LoadException(__FILE__, __LINE__, "CFont files must end with an .fnt extension");
    }
...
// Implement CFont stuff
void CFont::setSize( const int x, const int y ){
    // We don't change sizes
    if  ( (x < 0 || y < 0) && type!=Fixed ){
        return;
    }
}
int CFont::getSizeX() const{
    return width;
}
int CFont::getSizeY() const{
    return height;
}

int CFont::textLength( const char * text ) const{
    std::string str(text);
    int size =0;
...
int CFont::getHeight( const string & str ) const {
...
#if 0
void CFont::printf( int x, int y, int xSize, int ySize, Graphics::Color color, const Graphics::Bitmap & work, const string & str, int marker, ... ) const {
    /* call the other printf somehow.. */
}
#endif

void CFont::printf(int x, int y, int bank, const Graphics::Bitmap & work, const string & str, int marker, ... ){
    // Va list
    char buf[512];
    va_list ap;
...
void CFont::render(int x, int y, int position, int bank, const Graphics::Bitmap & work, const string & str){
...
unsigned char * CFont::findBankPalette(int bank) const {
    return pcx + pcxsize - ((bank+1) * colors * 3);
}

Graphics::Bitmap * CFont::makeBank(int bank) const {
...
PaintownUtil::ReferenceCount<Graphics::Bitmap> CFont::changeBank(int bank){
...
void CFont::load(const Filesystem::AbsolutePath & path){
